...
function isTimeInPeriods(currentTime: string, periods: { open: string; close: string }[]): boolean {
  return periods.some(period => currentTime >= period.open && currentTime <= period.close);
}

// Helper function to get next opening time from periods
function getNextOpeningFromPeriods(currentTime: string, periods: { open: string; close: string }[]): string | null {
...
export function getNextOpeningTime(now: Date): { day: string; time: string; isToday: boolean } | null {
  ...
  if (todayHours && todayHours.isOpen) {
    const nextOpeningToday = getNextOpeningFromPeriods(currentTime, todayHours.periods);
    if (nextOpeningToday) {
      return {
        day: currentDayName,
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (nextDayHours && nextDayHours.isOpen && nextDayHours.periods.length > 0) {
      // Get the first opening time of the day
      const firstOpening = nextDayHours.periods.sort((a, b) => a.open.localeCompare(b.open))[0];
      return {
        day: nextDayName,
...
export function formatNextOpeningTime(now: Date, t: (key: string, params?: Record<string, string | number | Date>) => string): string {
